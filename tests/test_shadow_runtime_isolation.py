...
class _Guard:
    def __init__(self, codes):
        self.state = SimpleNamespace(checked_at=10**12, issues=[_blocked_issue(code) for code in codes])
...
def test_shadow_manual_position_only_blockers_allow_hypothetical_scan():
    ...
    engine = SimpleNamespace(integrity=guard, paper_trade=False, _validation_safety_lock_active=True)
    iso.install_for_engine(engine, SimpleNamespace(warning=lambda *a, **k: None))
...
def test_isolation_does_not_apply_without_validation_lock():
    ...
    engine = SimpleNamespace(integrity=guard, paper_trade=False, _validation_safety_lock_active=False)
    iso.install_for_engine(engine, SimpleNamespace(warning=lambda *a, **k: None))
    assert guard.can_open_new() is False
    ...
    try:
        with patch.dict(os.environ, {"PAPER_TRADE": "false"}, clear=False):
            ...
            record = logging.getLogRecordFactory()(
                ...
                "🔴 OPERAÇÃO REAL ATIVA — ordens serão enviadas à KuCoin", (), None,
            )
