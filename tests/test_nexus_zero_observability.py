...
def test_zero_veto_is_classified_as_data_gate():
    log = _Log()
    nz.observe(_decision("Qualidade de dados 42/100 abaixo do mínimo (60)", dq=42), log)
    assert len(log.info_calls) == 1
    assert log.info_calls[0][2] == "DATA_GATE"
    assert log.info_calls[0][3] == 42.0


def test_zero_veto_is_classified_as_mtf_gate():
    log = _Log()
    nz.observe(_decision("Conflito entre timeframes (4H LONG / 1H SHORT) — seção 3"), log)
    assert len(log.info_calls) == 1
    assert log.info_calls[0][2] == "MTF_GATE"


def test_nonzero_or_approved_decisions_do_not_emit_zero_diagnostic():
    for decision in (
