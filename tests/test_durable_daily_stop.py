...
class DailyStopTests(unittest.IsolatedAsyncioTestCase):
    async def test_legacy_observed_stop_migration_no_longer_fabricates_stop(self):
        ...
        with patch.dict('os.environ', env), patch.object(gate.db, '_is_pg', True), patch.object(gate.db, 'configured_postgres_unavailable', return_value=False), patch.object(gate.db, 'load_key_value', new_callable=AsyncMock, return_value=None) as read, patch.object(gate.db, 'save_key_value', new_callable=AsyncMock, return_value=True) as write:
            engine = SimpleNamespace(daily_stopped=False)
            ...
            self.assertTrue(loaded_key.startswith('daily_stop_v2:'))
    ...
    def test_key_stable_across_deployments(self):
        ...
        self.assertTrue(a.startswith('daily_stop_v2:'))
