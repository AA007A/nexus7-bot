...
| P0-21 | P0 | `simulate_production_exit`, candidate/portfolio statistics | A position still open at the research cap or at data end was closed at the last close and given an R | Artificial trades entered expectancy, cost stress, ablation, thresholds, bootstrap and the portfolio | Censored ≠ completed trade | `RIGHT_CENSORED_*` status, `realized_r = null`, no manufactured leg or fee. Marked R and bounds are diagnostic. Predeclared materiality rule ⇒ `CENSORING_MATERIAL` | `CensoredOutcomesAreNotTrades` (4) | Bounds assume exits at the native SL/TP (gaps ignored). |
| P0-22 | P0 | replay windows | Decisions ran up to the data end | Recent decisions had no time to resolve | WARMUP + DECISION_WINDOW + OUTCOME_LOOKFORWARD | 30-day look-forward pinned in the manifest (≥ the research cap); the latest decision is moved back | `OutcomeWindows` (2) | none |
| P0-23 | P0 | `nexus_oos_inference` authority | 24/48/72 h blocks with a 10 h overlap assumption | Blocks shorter than the real outcome horizon carried authority | Block length ≥ max resolved horizon and ≥ 30 resampling blocks | Horizon-aware authority (`HORIZON_AWARE_BLOCK_BOOTSTRAP_V2`); gate re-derives the horizon; `AUTHORITY_BLOCK_SHORTER_THAN_OUTCOME_HORIZON` / `INSUFFICIENT_RESAMPLING_BLOCKS` | `HorizonAwareAuthority` (5) | Long horizons can make 180 days INSUFFICIENT_EVIDENCE; that is the honest result. |
| P0-24 | P0 | `purged_split` | 10 h embargo floor; censored rows had a finite end | Possible leakage with long or censored trades | Embargo ≥ longest resolved horizon; censored rows open-ended | Implemented | `LeakFreeSplitsWithLongAndCensoredTrades` | none |
| P0-25 | P0 | `path_bootstrap` | Spliced source-block futures into unrelated synthetic neighbours | Incoherent market timeline carried portfolio authority | No authority | `APPROXIMATE_NON_AUTHORITATIVE`; walk-forward calendar folds are the robustness authority | `PathBootstrapHasNoAuthority` (2) | Four folds over 180 days is a weak test; it can only block. |
| P0-26 | P0 | portfolio end state | Engine required every position to be closed | Censored positions were force-realized | Open, marked, bounded | `end_state` + `PORTFOLIO_CENSORING_MATERIAL` | `PortfolioEndStateKeepsUnresolvedPositionsOpen` (3) | none |
| P0-27 | P0 | policy parity (superseded by P0-33..P0-40) | Manifest only proven equal to the CI runtime | Manifest ≠ LIVE could go unnoticed | sha256-verified live attestation | `bot/policy_attestation.py` (whitelist only), startup log line, replay comparison. Pending ⇒ research gate allowed; `LIVE_RELEASE_GATE` requires `ATTESTED_MATCH` (P0-32) | `LivePolicyAttestation` (5) | Needs a deploy of this code to produce the LIVE line (stage B); not done in this phase. |
| P0-28 | P0 | candidate `temporal_folds` | Contiguous folds by row count | A fold-K outcome could use fold-K+1 market time | Calendar folds: decision + outcome completion (H) + embargo (≥ H); boundary decisions purged; censored excluded | `inf.purged_calendar_folds`, `replay.temporal_folds`, per-fold report, overlap assertion | `CandidateFoldsDoNotShareMarketTime` (3) | Long horizons can leave too little decision time: `INSUFFICIENT_INDEPENDENT_FOLDS`. |
| P0-29 | P0 | portfolio `walk_forward` | Contiguous folds; positions ran into the next fold | Fold results not independent | Same layout; trades truncated at the fold's outcome window (`RIGHT_CENSORED_FOLD_END`) | `pe.walk_forward_folds`; `INSUFFICIENT_INDEPENDENT_PORTFOLIO_PERIODS` when four folds do not fit; embargo never reduced | `PortfolioFoldsCarryNothingForward`, `EmbargoGrowsWithTradeLength`, `InsufficientIndependentPeriods`, `EmbargoIsNeverShrunk` | none |
| P0-30 | P0 | walk-forward account state | Undeclared | Could be read as a continuous backtest | Declared | `fold_account_state = RESET_FOR_REGIME_ROBUSTNESS`, `not_a_continuous_backtest = true`; the gate requires the declaration | gate tests | none |
| P0-31 | P0 | promotion gate fold checks | Fold counts trusted from the artifact | Overlapping folds could pass | Recompute from dates | `folds_independent`: embargo ≥ gate horizon, gap ≥ H + E, `max_market_ts_used` < next start ⇒ `TEMPORAL_FOLDS_NOT_INDEPENDENT` / `PORTFOLIO_FOLDS_NOT_INDEPENDENT` | fold tests (above) | none |
| P0-32 | P0 | attestation vs gate (superseded by P0-33..P0-40) | The PR gate required an attestation that only a deploy can produce | Deadlock: the code that emits the line could never pass | Stages A/B/C | `RESEARCH_PROMOTION_GATE` (pending allowed, `production_ready` always false) vs `LIVE_RELEASE_GATE` (`ATTESTED_MATCH`, exact SHA, protection readiness, human authorization) | `StagedGates` (5) | Stage B is not created in this phase. |
| P1-22 | P1 | inference terminology | `independent_blocks` | Overclaimed independence | `resampling_blocks` | Renamed across inference, research, replay and gate | existing tests | none |
| P1-23 | P1 | residual dependence | Not measured | Blocks could still be dependent | Predeclared rule | Lag-1/2/3 ACF + band per length ≥ H; significant at the longest usable ⇒ no authority, no shorter substitute; the gate recomputes it from the per-block series (P1-24) | `ResidualDependenceFailsClosed` (2) | none |
| P0-33 | P0 | `policy_attestation` semantics | sha256 of plaintext was labelled `ATTESTED_MATCH` | Integrity presented as authentication | Content ≠ provenance | Renamed to `POLICY_CONTENT_MATCH` etc.; `provenance = UNAUTHENTICATED`; fixture-only replay input | `LivePolicyAttestation` (4), `FabricatedPayloadIsNotProvenance` (2) | none |
| P0-34 | P0 | runtime policy line | No code or deployment identity | A line could be reused for any deployment | V2 line with identity | `LIVE_POLICY_OBSERVATION_V2`: format, candidate_sha, deployment_id, hashed service/environment, generated_at | parse/identity tests | Values come from platform variables; provenance still needs Stage C |
| P0-35 | P0 | release evidence | No deployment-bound evidence | Evidence not tied to a deployment/SHA | `BGX_LIVE_RELEASE_EVIDENCE_V1` | `bot/live_release_evidence.py`: schema, parser, verifier against read-only `TrustedSources` | `tests/test_live_release_evidence.py` (22) | No production provider yet ⇒ BLOCK |
| P0-36 | P0 | stale / replayed evidence | Unlimited reuse | Old evidence could authorize a new release | Predeclared limits | Runtime observation 6 h and after deployment, approval 24 h, CI/protection 7 days, 5 min skew | `StaleEvidence` (4) | none |
| P0-37 | P0 | committed attestation file | Workflow read `research/live_policy_attestation.txt` | Fabricable, stale, no provenance | Never from the repo | Removed from workflow and paths; contract test forbids it | workflow contract test | none |
| P0-38 | P0 | protection readiness | The string `PASS` | Assertion, not evidence | Structured, digest-bound, exact-SHA runs | `protection` object verified via the CI provider | `ProtectionStringIsNotEvidence` (2) | none |
...
| STATISTICAL_AUTHORITY | Horizon-aware; the gate enforces the horizon, resampling blocks, residual dependence, fold independence and censoring materiality. |
| POLICY_PARITY | LIVE_POLICY_OBSERVATION_PENDING; content match is not provenance. LIVE_RELEASE_EVIDENCE = BLOCK (no trusted source provider). |
