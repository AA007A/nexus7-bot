...
| P0-27 | P0 | policy parity | Manifest only proven equal to the CI runtime | Manifest ≠ LIVE could go unnoticed | sha256-verified live attestation | `bot/policy_attestation.py` (whitelist only), startup log line, replay comparison. Pending ⇒ research gate allowed; `LIVE_RELEASE_GATE` requires `ATTESTED_MATCH` (P0-32) | `LivePolicyAttestation` (5) | Needs a deploy of this code to produce the LIVE line (stage B); not done in this phase. |
...
| P1-23 | P1 | residual dependence | Not measured | Blocks could still be dependent | Predeclared rule | Lag-1/2/3 ACF + band per length ≥ H; significant at the longest usable ⇒ no authority, no shorter substitute; the gate recomputes it | `ResidualDependenceFailsClosed` (2) | none |
...
| Full offline suite (`python -m tests.run_offline`), final code | 1827/1827 pass (1726 before the parity phase) |
...
| POLICY_PARITY | LIVE_ATTESTATION_PENDING (no LIVE attestation line yet; stage B not started). |
