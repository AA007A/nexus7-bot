...
| P0-23 | P0 | `nexus_oos_inference` authority | 24/48/72 h blocks with a 10 h overlap assumption | Blocks shorter than the real outcome horizon carried authority | Block length ≥ max resolved horizon and ≥ 30 independent blocks | Horizon-aware authority (`HORIZON_AWARE_BLOCK_BOOTSTRAP_V2`); gate re-derives the horizon; `AUTHORITY_BLOCK_SHORTER_THAN_OUTCOME_HORIZON` / `INSUFFICIENT_INDEPENDENT_BLOCKS` | `HorizonAwareAuthority` (5) | Long horizons can make 180 days INSUFFICIENT_EVIDENCE; that is the honest result. |
...
| P0-27 | P0 | policy parity | Manifest only proven equal to the CI runtime | Manifest ≠ LIVE could go unnoticed | sha256-verified live attestation | `bot/policy_attestation.py` (whitelist only), startup log line, replay comparison, `LIVE_POLICY_NOT_ATTESTED` gate blocker | `LivePolicyAttestation` (5) | Needs a deploy of this code to produce the LIVE line; not done in this phase. |
...
| STATISTICAL_AUTHORITY | Horizon-aware; the gate enforces the horizon, independent blocks and censoring materiality. |
| POLICY_PARITY | PRODUCTION_REPORTED_NOT_CRYPTOGRAPHICALLY_ATTESTED (no LIVE attestation line yet). |
