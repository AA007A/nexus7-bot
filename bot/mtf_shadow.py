...
def _remember_unique(key):
    with _LOCK:
        ...
        _METRICS["unique_states"] += 1
        return True


def _record_block(stage, symbol, extra=None):
    with _LOCK:
        _METRICS["shadow_blocks"][stage] += 1
        _METRICS["symbols"][symbol] += 1
        if extra:
            _METRICS["last"].append(extra)
...
def _nexus_reason(dec):
    try:
        if dec.execution_allowed is True:
            return "approved"
        reasoning = getattr(dec, "reasoning", None) or []
        if reasoning:
            return str(reasoning[-1])[:160]
        warnings = getattr(dec, "warnings", None) or []
        if warnings:
            return str(warnings[-1])[:160]
...
def observe(symbol, k15, k1h, k4h, production_result=None,
    ...
    with _LOCK:
        if production_result is not None:
            _METRICS["policy_a_signals"] += 1
        else:
            _METRICS["policy_a_holds"] += 1

    # We only test the relaxation when Policy A held.
    if production_result is not None:
        return

    try:
        if len(k4h) < 10 or len(k1h) < 15 or len(k15) < 20:
            _record_block("DATA", symbol)
            return

        c4h,h4h,l4h,o4h,v4h = _ga(k4h[:-1] if len(k4h) > 10 else k4h)
        c1h,h1h,l1h,o1h,v1h = _ga(k1h[:-1] if len(k1h) > 15 else k1h)
        c15,h15,l15,o15,v15 = _ga(k15[:-1] if len(k15) > 20 else k15)
        price = c15[-1]

        atr_4h, avg_4h = _get_atr(h4h,l4h,c4h)
        atr_1h, avg_1h = _get_atr(h1h,l1h,c1h)
        atr_15, avg_15 = _get_atr(h15,l15,c15)

        regime = detect_regime(c4h,h4h,l4h,atr_4h)
        if regime in ("COMPRESSED","RANGING","CHOPPY"):
            _record_block("REGIME_4H", symbol)
            return

        e20_4h = float(ema(c4h,20)[-1]); e50_4h = float(ema(c4h,50)[-1])
        e20_1h = float(ema(c1h,20)[-1]); e50_1h = float(ema(c1h,50)[-1])
        bull4 = e20_4h > e50_4h and c4h[-1] > e20_4h
        bear4 = e20_4h < e50_4h and c4h[-1] < e20_4h
        ...
        if bull4:
            ...
            _record_block("MTF_4H_NEUTRAL", symbol)
            return

        with _LOCK:
            _METRICS["eligible_4h_dir_1h_neutral"] += 1

        s4 = score_tf(c4h,h4h,l4h,o4h,v4h,direction,atr_4h,avg_4h)
        s1 = score_tf(c1h,h1h,l1h,o1h,v1h,direction,atr_1h,avg_1h)
        s15 = score_tf(c15,h15,l15,o15,v15,direction,atr_15,avg_15)
        if not s4.get("ok") or not s1.get("ok") or not s15.get("ok"):
            _record_block("SCORE_INVALID", symbol)
            return

        combined = round(s4["total"]*0.25 + s1["total"]*0.30 + s15["total"]*0.45)
        if combined < min_score:
            _record_block("SCORE", symbol)
            return
        if s15["rsi_v"] > 92 or s15["rsi_v"] < 8:
            _record_block("RSI", symbol)
            return
        if s15["vol_r"] < 0.40:
            _record_block("VOLUME", symbol)
            return
        if not s15["aligned"] and regime not in ("TRENDING_UP","TRENDING_DOWN"):
            _record_block("ALIGN_15M", symbol)
            return
        ...
        if not entry_ok:
            ...
            if combined < min_score:
                _record_block("ENTRY_TRIGGER", symbol)
                return
        ...
        if rr < cfg.MIN_RR_RATIO:
            _record_block("RR", symbol)
            return

        cost_pct = TOTAL_COST * 100
        move_to_tp = abs(tp-price)/price*100
        if move_to_tp < cost_pct * fee_mult:
            _record_block("COSTS", symbol)
            return
        ...
        dec = nexus_ai.decide(
            ...
            min_score=float(os.environ.get("NEXUS_MIN_SCORE", "55")),
        )
...
def snapshot():
    with _LOCK:
        ...
        return {
            ...
            "policy_b": "4H directional + 1H neutral allowed; opposite blocked",
            "started_at": _METRICS["started_at"],
            "uptime_minutes": round((time.time()-_METRICS["started_at"])/60,1),
            "raw_calls": _METRICS["raw_calls"],
            "unique_states": unique,
            "policy_a_signals": _METRICS["policy_a_signals"],
            "policy_a_holds": _METRICS["policy_a_holds"],
            "eligible_4h_dir_1h_neutral": eligible,
            "eligible_pct": round(eligible/unique*100,2) if unique else 0.0,
            "blocked_1h_opposite": _METRICS["blocked_1h_opposite"],
            "shadow_pre_ai_survivors": survivors,
            "survival_pct_of_eligible": round(survivors/eligible*100,2) if eligible else 0.0,
            "shadow_nexus_approved": _METRICS["shadow_nexus_approved"],
            "shadow_nexus_vetoed": _METRICS["shadow_nexus_vetoed"],
            "nexus_approval_pct": round(_METRICS["shadow_nexus_approved"]/nx_total*100,2) if nx_total else 0.0,
