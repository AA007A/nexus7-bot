...
Centralizes legacy infrastructure/observability monkey patches previously
implemented inline by sitecustomize. Behavior is intentionally preserved.
"""
...
def install(*, TradingEngine, Analyzer, nexus_persistence, funnel_metrics,
            ...
            notifier, log) -> None:
    np, fm, ms = nexus_persistence, funnel_metrics, mtf_shadow

    if not getattr(np, "_single_conn_serialized", False):
        orig_execute, orig_fetchall = np._execute, np._fetchall
        ...
        np._execute, np._fetchall = execute_serialized, fetchall_serialized
    ...
    if not getattr(Analyzer, "_mtf_shadow_patched", False):
        ...
        def analyze_mtf_with_shadow(self, symbol, k15, k1h, k4h,
            ...
            result = orig_analyze_mtf(self, symbol, k15, k1h, k4h,
                                      min_score=min_score, fee_mult=fee_mult,
                                      vol_mult=vol_mult)
            try:
                ...
                snap = ms.snapshot(); unique = snap.get("unique_states", 0)
                if unique != before and (unique == 1 or unique % 25 == 0):
                    log.info("[MTF_SHADOW] unique=%s eligible=%s survivors=%s nexus_approved=%s nexus_vetoed=%s execution_effect=NONE",
