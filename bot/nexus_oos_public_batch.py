...
so simultaneous cross-asset candidates remain dependent. Robustness diagnostics
use the exact same requested symbol universe and have BLOCK-ONLY authority: they
may prevent promotion when edge is temporally unstable or concentrated, but can
never authorize AI_EDGE_PROVEN by themselves.

Because historical OI and order-book snapshots are not available in the current
public replay, this entrypoint remains fail-closed with
HISTORICAL_CONTEXT_PARITY_INCOMPLETE until every successful symbol explicitly
proves full context parity.
"""
...
async def run_public_batch(
    ...
    *,
    ...
    successful_reports = [report for report in reports if not report.get("error")]
    robustness = analyze_robustness(successful_reports, temporal_folds=int(temporal_folds))
    robustness_blockers = robustness_promotion_blockers(
        robustness,
        min_temporal_folds=int(temporal_folds),
        min_temporal_positive_fraction=0.75,
        require_all_leave_one_symbol_out_positive=True,
    )

    blockers = set(payload.get("blockers") or ())
    blockers.update(robustness_blockers)
    ...
    # Full context parity, statistical positivity and robustness all have to
    # pass. Robustness is block-only and can never promote a failing result.
    ...
    payload["robustness_blockers"] = list(robustness_blockers)
    ...
    payload["methodology"] = {
        ...
        "robustness_symbol_universe": "same_as_requested_batch",
        "robustness_temporal_folds": int(temporal_folds),
        "robustness_min_positive_fold_fraction": 0.75,
        "robustness_leave_one_symbol_out": "all_positive_required",
        "robustness_promotion_role": "BLOCK_ONLY",
    ...
    (destination / "nexus_oos_robustness.json").write_text(
        json.dumps(
            {
                "schema_version": 1,
                "status": "ROBUSTNESS_BLOCK_ONLY",
                "blockers": list(robustness_blockers),
                "requested_symbols": list(requested),
                "successful_symbols": [replay.symbol for replay in replays],
