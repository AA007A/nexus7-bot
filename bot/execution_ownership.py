...
def _lease_deadline(ownership: ExecutionOwnership) -> datetime:
    deadline = _parse(str(ownership.expires_at))
    return deadline if deadline.tzinfo is not None else deadline.replace(tzinfo=timezone.utc)

def _ownership_state_log(engine, *, event: str, db_lease_valid: bool, fencing_valid: bool, reason: str) -> None:
    ...
    log.info(
        ...
        "execution_ownership_valid=%s fencing_valid=%s lease_remaining_seconds=%.1f reason=%s",
        event, str(db_lease_valid).lower(), str(local_lease_valid).lower(),
        str(bool(getattr(engine, "_execution_ownership_valid", False))).lower(),
        str(fencing_valid).lower(), remaining, reason,
    )

def publish_validated_execution_ownership(engine, ownership: ExecutionOwnership, *, event: str) -> None:
...
async def initialize_live_execution_ownership(engine):
    ...
    """
    if current_execution_capability() is not ExecutionCapability.LIVE:
        invalidate_execution_ownership(engine, event="local_invalidated", reason="capability_not_live")
        return None
    ownership = await acquire_execution_ownership()
    await validate_execution_ownership(ownership)
    publish_validated_execution_ownership(engine, ownership, event="startup_validated")
    from bot.logger import log
...
async def execution_ownership_heartbeat(engine):
    ...
    while getattr(engine, "_running", False):
        try:
            ownership = await acquire_execution_ownership()
            await validate_execution_ownership(ownership)
            publish_validated_execution_ownership(engine, ownership, event="heartbeat_renewed")
            log.info("[EXECUTION_OWNERSHIP] heartbeat_renewed=true fencing_valid=true lease_seconds=%s", _LEASE_SECONDS)
        except Exception as exc:
            invalidate_execution_ownership(engine, event="heartbeat_failed", reason=type(exc).__name__)
