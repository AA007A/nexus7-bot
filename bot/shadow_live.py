...
async def connect_readonly(engine) -> bool:
    """Bring up only the read-only subset required for live shadow analysis."""
    try:
        ...
        if balance < 0:
            log.error("[SHADOW_LIVE] authenticated balance read failed")
            engine.connected = False
            engine.active = False
            return False

        engine.risk.init(balance)
        engine.risk.update(balance)
        ...
        if not engine.viable_symbols:
            log.warning("[SHADOW_LIVE] no viable symbols; shadow scan remains blocked")
            engine.connected = False
            engine.active = False
            return False

        # Public market-data WS only. Do not start private order WS here.
        ws_symbols = engine.viable_symbols[:30]
        try:
            await asyncio.wait_for(
                engine.client.start_websocket(ws_symbols, intervals=["15", "60", "240"]),
                timeout=10,
            )
        ...
        engine.active = True
        log.warning(
            "[SHADOW_LIVE] READY balance_read=true viable_symbols=%s "
...
async def evaluate_candidate(engine, sig):
    """Evaluate a real-market candidate through the pre-dispatch gates only."""
    try:
        ...
        if getattr(engine, "_durable_state_enforced", False):
            ...
            if not durable.can_open(engine):
                log.warning("[SHADOW_LIVE] %s blocked: durable state not confirmed", sig.symbol)
                return None
        ...
        log.info(
            "[SHADOW_AI] symbol=%s side=%s decision=%s approved=%s source=%s reason=%s execution_effect=NONE",
            sig.symbol, sig.direction, "APPROVE" if approved else "REJECT",
            approved, decision_source, reason or "approved",
        )
        if not approved:
            return None

        if not await engine._refresh_entry_balance():
            log.warning("[SHADOW_LIVE] %s blocked: balance refresh failed", sig.symbol)
            return None
        fresh_bal = float(engine.risk.balance or 0)

        # Match the controlled-pilot sizing rule when pilot is effective.
        if engine.pilot.enabled:
            ...
            if fresh_bal <= 0 or required > fresh_bal:
                log.warning("[SHADOW_LIVE] %s blocked: minimum lot exceeds available balance", sig.symbol)
                return None
        else:
            qty = engine.risk.size(
                sig.symbol, sig.entry, engine.instruments,
                open_positions=engine.positions,
            )
        if qty <= 0:
            log.warning("[SHADOW_LIVE] %s blocked: qty=0", sig.symbol)
            return None

        # Pre-trade market-data score, using the same real inputs as LIVE.
        ...
        if len(kl) < 20:
            try:
                ...
                kl = []
        if len(kl) <= 20:
            log.warning("[SHADOW_LIVE] %s blocked: insufficient closed candles", sig.symbol)
            return None
        ...
        if not pre_score.get("aprovado"):
            log.info(
                "[SHADOW_LIVE] %s blocked: pretrade=%s minimum=%s execution_effect=NONE",
                sig.symbol, pre_score.get("total"), scoring.MIN_SCORE,
            )
            return None

        info = engine.instruments.get(sig.symbol, {})
        validate_base_quantity(qty, info, sig.entry)
        if sig.sl <= 0 or sig.tp <= 0:
            log.warning("[SHADOW_LIVE] %s blocked: invalid SL/TP", sig.symbol)
            return None
        if sig.direction == "LONG" and (sig.sl >= sig.entry or sig.tp <= sig.entry):
            log.warning("[SHADOW_LIVE] %s blocked: LONG SL/TP side invalid", sig.symbol)
            return None
        if sig.direction == "SHORT" and (sig.sl <= sig.entry or sig.tp >= sig.entry):
            log.warning("[SHADOW_LIVE] %s blocked: SHORT SL/TP side invalid", sig.symbol)
            return None
        ...
        liq_result = liq.analyze(
            ...
            symbol=sig.symbol,
            n_open_positions=len(engine.positions) + 1,
        )
        if not liq_result.stop_effective:
            log.warning(
                "[SHADOW_LIVE] %s blocked: liquidation_guard reason=%s execution_effect=NONE",
                sig.symbol, liq_result.reason,
            )
            return None
        ...
        log.warning(
            ...
            "qty=%.12g notional=%.4f score=%s pretrade=%s pilot_blockers=%s execution_effect=NONE",
            sig.symbol, sig.direction, sig.entry, sig.sl, sig.tp, qty,
            qty * sig.entry, sig.score, pre_score.get("total", 0), len(pilot_blockers),
        )
        return record
    except Exception as exc:
