...
class Config:
    ...
    # Trading — pares de alta liquidez e confiabilidade
    SYMBOLS: list = [
    ...
    MAX_RISK_PCT:    float = float(os.environ.get("MAX_RISK_PCT", "0.35"))  # 35% do poder por trade
    MAX_DRAWDOWN:    float = float(os.environ.get("MAX_DRAWDOWN", "0.80"))  # 80% DD máximo
    MAX_POSITIONS:   int   = int(os.environ.get("MAX_POSITIONS",  "4"))     # até 4 posições
    MIN_CONFIDENCE:  float = float(os.environ.get("MIN_CONFIDENCE","0.75")) # 75% confiança
