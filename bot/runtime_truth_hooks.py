...
def _checkpoint_snapshot(client) -> dict:
    ...
    return {
        "cache_state": cache_state, "provenance": provenance,
        "cache_data_hashes": cache_hashes, "provenance_hashes": provenance_hashes,
    }
...
def _raw_ws_fields(msg: dict) -> tuple[str | None, str | None, int | None, dict]:
    ...
    ts = int(float(candles[0])); ts = ts * 1000 if ts < 100_000_000_000 else ts
    return symbol, timeframe, ts, {"ws_index_5_volume":candles[5], "ws_index_6_amount":candles[6]}


def _mutation_action(before: list[dict], after: list[dict], candle_ts: int | None) -> str:
    if candle_ts is None: return "OTHER"
    before_ts = [int(x.get("ts", -1)) for x in before]
    after_ts = [int(x.get("ts", -1)) for x in after]
    if candle_ts in before_ts and candle_ts in after_ts: return "REPLACE_EXISTING"
    if after_ts and candle_ts == after_ts[-1]: return "APPEND"
    return "OUT_OF_ORDER_INSERT"


def _register_series_provenance(symbol: str, timeframe: str, rows: list[dict], source: str, raw_event_id: str | None, raw_volume_fields: dict | None = None):
    for row in rows:
        if row.get("ts") is None: continue
        truth.RECORDER.register_provenance(symbol, timeframe, int(row["ts"]), {
            "candle_ts": int(row["ts"]), "source": source,
            "raw_event_id": raw_event_id, "raw_volume_fields": dict(raw_volume_fields or {}),
...
def install_transport_and_cache(KuCoinClient):
    ...
    class WSProxy:
        def __init__(self, ws, session_id: str, capture: bool):
            self._ws=ws; self._session_id=session_id; self._capture=capture; self._seen_data=False
        def __getattr__(self, name): return getattr(self._ws, name)
            ...
            if self._capture:
                try:
                    ...
                    if not self._seen_data:
                        self._seen_data=True; truth.request_checkpoint("WS_RECONNECT_STABILIZED")
                except Exception: pass
            return raw
    class ConnectProxy:
        def __init__(self, inner, capture: bool):
            self._inner=inner; self._capture=capture; self._session_id=str(uuid.uuid4()); self._ws=None
        async def __aenter__(self):
            global _PREVIOUS_WS_SESSION
            ws = await self._inner.__aenter__(); self._ws=WSProxy(ws,self._session_id,self._capture)
            if self._capture:
                truth.emit("MARKET_WS_RECONNECT", payload={
                    ...
                    "reconnect_timestamp":time.time(), "source":"CONNECT",
                })
                _PREVIOUS_WS_SESSION=self._session_id
            return self._ws
        async def __aexit__(self,*args): return await self._inner.__aexit__(*args)
    def connect_with_truth(uri, *args, **kwargs):
        text=str(uri); capture=("connectId=bgx-" in text and "bgx7-priv" not in text)
    ...
    async def seed_with_truth(self, symbols, intervals):
        ...
        truth.emit("MARKET_CACHE_RESEED", payload={"reason":"STARTUP_SEED_COMPLETE","series_count":len(getattr(self,"_kline_cache",{})),"sequence_watermark":truth.RECORDER.current_sequence})
    ...
    async def get_klines_with_truth(self, symbol, interval, limit=200):
        key=(symbol,str(interval)); before=[dict(x) for x in list((getattr(self,"_kline_cache",{}) or {}).get(key,[]))]
        ...
        source = truth.current_rest_purpose(); raw_id = _LAST_REST_EVENT_ID.get()
        truth.emit("MARKET_CACHE_MUTATION", symbol=symbol, timeframe=str(interval), payload={
            "cache_key":f"{symbol}|{interval}", "CACHE_BEFORE_HASH":truth.cache_data_hash(before),
            "CACHE_AFTER_HASH":truth.cache_data_hash(after), "mutation_action":"CACHE_EXTEND",
            "source":source, "raw_event_id":raw_id, "normalized_row_before":before[-1] if before else None,
            "normalized_row_after":after[-1] if after else None, "series_after":after,
        ...
        _register_series_provenance(symbol,str(interval),after,source,raw_id)
        return result
    ...
    async def ws_handler_with_truth(self, msg):
        ...
        if key:
            ...
            raw_id=_LAST_WS_EVENT_ID.get(); action=_mutation_action(before,after,candle_ts)
            truth.capture_cache_mutation(symbol=symbol,timeframe=str(timeframe),before_rows=before,after_rows=after,
                mutation_action=action,source="WS_UPDATE",raw_event_id=raw_id,
                normalized_before=row_before,normalized_after=row_after,raw_volume_fields=raw_fields)
        return result
...
def install_marketdata_outer(Analyzer, TradingEngine):
    if not truth.enabled() or getattr(Analyzer,"_runtime_truth_marketdata_outer",False): return
    original=Analyzer.analyze_mtf
    def wrapped(self,symbol,k15,k1h,k4h,*args,**kwargs):
        evaluation_id=truth.new_evaluation_id("POLICY_A_STRATEGY",symbol); client=_ACTIVE_CLIENT.get()
        ...
        with truth.evaluation_context(evaluation_id,"POLICY_A_STRATEGY",meta):
            ...
            _remember_signal(result)
            return result
    Analyzer.analyze_mtf=wrapped; Analyzer._runtime_truth_marketdata_outer=True
...
def install_engine_and_downstream(TradingEngine, KuCoinClient, scoring, nexus_ai):
    if not truth.enabled() or getattr(TradingEngine,"_runtime_truth_downstream_installed",False): return
    ...
    async def run_with_truth(self,*args,**kwargs):
        ...
        try: return await original_run(self,*args,**kwargs)
        finally: await runtime_truth_exporter.stop()
    TradingEngine.run=run_with_truth

    original_open=TradingEngine._open
    async def open_with_truth(self,sig,*args,**kwargs):
        evaluation_id,scope=_SIGNAL_EVALUATIONS.get(id(sig),(truth.new_evaluation_id("ENGINE_CANDIDATE",getattr(sig,"symbol",None)),"POLICY_A_STRATEGY"))
        with truth.evaluation_context(evaluation_id,scope,{}):
            truth.emit_stage_result("ENGINE_CANDIDATE_RESULT","ENGINE_CANDIDATE",sig,symbol=getattr(sig,"symbol",None))
        ...
        try:
            with truth.evaluation_context(evaluation_id,"NEXUS",{}), truth.rest_purpose("REST_FALLBACK"):
                ...
                truth.emit("MARKET_ANALYSIS_RESULT",symbol=getattr(sig,"symbol",None),payload={
                    "evaluation_id":evaluation_id,"evaluation_scope":"NEXUS","stage":"NEXUS_RESULT",
                    "authority":"NEXUS_APPROVED" if allowed else "NEXUS_REJECTED","execution_allowed":allowed,
                    "decision":getattr(result,"decision",None),"confidence":getattr(result,"confidence",None),
                    "setup_quality":getattr(result,"setup_quality",None),
                })
                return result
        finally: _ACTIVE_CLIENT.reset(client_token)
    TradingEngine._nexus_validate=nexus_validate_with_truth

    original_decide=nexus_ai.decide
    def decide_with_truth(*args,**kwargs):
        symbol=kwargs.get("symbol") or (args[0] if args else None); evaluation_id,scope=truth.current_evaluation()
        if scope=="NEXUS":
            tfs={}
            for tf,name in (("15","k15"),("60","k1h"),("240","k4h")):
                rows=list(kwargs.get(name) or [])
                tfs[tf]={"NEXUS_RAW_INPUT_HASH":truth.cache_data_hash(rows),"row_count":len(rows),"last_timestamp":rows[-1].get("ts") if rows else None}
            truth.emit("MARKET_ANALYSIS_INPUT",symbol=symbol,payload={"evaluation_id":evaluation_id,"evaluation_scope":"NEXUS","timeframes":tfs})
    ...
    async def calculate_with_truth(*args,**kwargs):
        result=await original_calculate(*args,**kwargs); evaluation_id,scope=truth.current_evaluation(); symbol=args[0] if args else kwargs.get("symbol")
        if evaluation_id:
            approved=bool(result.get("aprovado",False)) if isinstance(result,dict) else False
            truth.emit("MARKET_ANALYSIS_RESULT",symbol=symbol,payload={"evaluation_id":evaluation_id,"evaluation_scope":scope,"stage":"RISK_PRETRADE_RESULT","authority":"PRETRADE_APPROVED" if approved else "PRETRADE_REJECTED","result":copy.deepcopy(result) if isinstance(result,dict) else str(result)})
        return result
    ...
    async def place_order_with_truth(self,*args,**kwargs):
        result=await original_place_order(self,*args,**kwargs); evaluation_id,scope=truth.current_evaluation(); symbol=kwargs.get("symbol") or (args[0] if args else None)
        if evaluation_id:
            dispatched=bool(isinstance(result,dict) and result.get("orderId"))
            truth.emit("MARKET_ANALYSIS_RESULT",symbol=symbol,payload={"evaluation_id":evaluation_id,"evaluation_scope":scope,"stage":"EXCHANGE_DISPATCH_RESULT","authority":"EXCHANGE_DISPATCHED" if dispatched else "NOT_DISPATCHED","reduce_only":bool(kwargs.get("reduce_only",False)),"order_id_present":dispatched})
        return result
    ...
    async def wait_with_truth(self,order_id,*args,**kwargs):
        result=await original_wait(self,order_id,*args,**kwargs); evaluation_id,scope=truth.current_evaluation()
        if evaluation_id:
            filled=bool(result.get("filled",False)) if isinstance(result,dict) else False
            truth.emit("MARKET_ANALYSIS_RESULT",payload={"evaluation_id":evaluation_id,"evaluation_scope":scope,"stage":"FILL_RESULT","authority":"FILLED" if filled else "NOT_FILLED","filled":filled,"timed_out":result.get("timed_out") if isinstance(result,dict) else None})
        return result
