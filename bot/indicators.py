...
def rsi(closes: list, period: int = 14) -> np.ndarray:
    """
    RSI com Wilder Smoothing (método correto).
    Retorna array completo com valor válido para TODOS os índices >= period.
    Índices anteriores ao period inicial retornam 50.0 (neutro).

    FIX: versão anterior calculava apenas out[-1] e fixava out[:-1]=50,
    tornando o histórico inutilizável. Esta versão calcula a série completa.
    """
    arr    = np.array(closes, dtype=float)
    n      = len(arr)
    out    = np.full(n, 50.0)   # padrão neutro para índices iniciais
    ...
    # Semente: média simples dos primeiros `period` ganhos/perdas
    ag = gains[:period].mean()
    al = losses[:period].mean()

    # Primeiro RSI válido no índice `period`
    rs = ag / al if al > 0 else 1e9
    out[period] = 100.0 - 100.0 / (1.0 + rs)

    # Propaga com Wilder Smoothing para todos os candles subsequentes
...
# ── VWAP ────────────────────────────────────────────────────────
def vwap(highs: list, lows: list, closes: list, volumes: list,
         timestamps: list = None, anchor: str = "session") -> float:
    """
    VWAP com âncora de sessão diária (00:00 UTC).

    LOGIC-2: Versão anterior acumulava TODOS os candles históricos,
    distorcendo o VWAP para longe do preço atual e tornando-o inútil
    como nível de referência intraday.

    Solução: âncora no início da sessão UTC (meia-noite).
    Se timestamps fornecidos: usa apenas candles da sessão atual.
    Se não fornecidos: usa os últimos 96 candles (≈ 1 dia em 15M).

    anchor:
      "session" → âncora diária às 00:00 UTC (padrão)
      "week"    → âncora semanal (segunda-feira 00:00 UTC)
      "all"     → comportamento legado (acumula tudo)
    """
    h = np.array(highs,   dtype=float)
    l = np.array(lows,    dtype=float)
    c = np.array(closes,  dtype=float)
    ...
    # Determina ponto de âncora
    if anchor == "all" or timestamps is None:
        # Fallback: usa últimos 96 candles (1 dia em 15M)
        window = min(96, len(c))
        h, l, c, v = h[-window:], l[-window:], c[-window:], v[-window:]
    elif anchor in ("session", "week") and timestamps:
        ...
        if anchor == "session":
            # Âncora: início do dia UTC atual
            anchor_dt = now_dt.replace(hour=0, minute=0, second=0, microsecond=0)
        else:  # week
        ...
        if mask.sum() < 2:
            # Sessão nova com poucos candles — usa todos disponíveis
    ...
    typical = (h + l + c) / 3
    total_v = np.sum(v)
    return float(np.sum(typical * v) / total_v) if total_v > 0 else float(c[-1])
